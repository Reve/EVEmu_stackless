...
class EVEMarshal:
	...
	def marshal(self, data):
		if type(data).__name__ == 'NoneType':
			...
			return self.marshalDict(data)
		elif type(data).__name__ == 'bytes':
			return self.marshalBytes(data)
		elif type(data).__name__ == 'bool':
			return self.marshalBool(data)
		else:
	...
	def marshalBytes(self, data):
		marshalledData = pack('<b', self.EVEMarshalOpCodes['buffer'])
		size = pack('<b', len(data))
		marshalledData = size + marshalledData
		return marshalledData

	def marshalBool(self, data):
		if data == True:
			marshalledData = pack('<b', self.EVEMarshalOpCodes['true'])
		else:
			marshalledData = pack('<b', self.EVEMarshalOpCodes['false'])
	...
	def marshalDict(self, data):
		marshalledData = pack('<b', self.EVEMarshalOpCodes['dict'])
		marshalledData += pack('<b', len(data))

		for k, v in data.items():
			marshalledData += self.marshal(v)
			marshalledData += self.marshal(k)
	...
	def unmarshal(self, data):
		if data[0] == self.EVEMarshalOpCodes['none']:
			...
			return marshalledData
		elif data[0] == self.EVEMarshalOpCodes['stringtableitem']:
			marshalledData = data[2:data[2]+1]
			return marshalledData
		elif data[0] == self.EVEMarshalOpCodes['utf8string']:
			marshalledData = data[2:data[2]+1]
			return marshalledData
		elif data[0] == self.EVEMarshalOpCodes['single_char_str']:
			marshalledData = chr(data[1:3])
			return marshalledData
		elif data[0] == self.EVEMarshalOpCodes['emptystring']:
			marshalledData = data[2:data[2]+1]
			return marshalledData
		elif data[0] == self.EVEMarshalOpCodes['shortstring']:
			marshalledData = data[2:data[2]+1]
			return marshalledData
		elif data[0] == self.EVEMarshalOpCodes['charstring']:
			marshalledData = data[2:data[2]+1]
			return marshalledData
		elif data[0] == self.EVEMarshalOpCodes['stringempty']:
			marshalledData = data[2:data[2]+1]
			return marshalledData
			...
			while element < numElements:
				...
				elif data[currentByte] == self.EVEMarshalOpCodes['longstring'] or data[currentByte] == self.EVEMarshalOpCodes['stringtableitem'] or data[currentByte] == self.EVEMarshalOpCodes['utf8string'] or data[currentByte] == self.EVEMarshalOpCodes['emptystring'] or data[currentByte] == self.EVEMarshalOpCodes['stringempty'] or data[currentByte] == self.EVEMarshalOpCodes['shortstring'] or data[currentByte] == self.EVEMarshalOpCodes['charstring']:
				...
				elif data[currentByte] == self.EVEMarshalOpCodes['single_char_str']:
					unmarshaledList.append(self.unmarshal(data[currentByte + 1]))
					currentByte += 3
		...
		elif data[0] == self.EVEMarshalOpCodes['object']:
			objType = data[1]
			arguments = self.unmarshal(data[2:])

			print(type(arguments), arguments)

			return None
		elif data[0] == self.EVEMarshalOpCodes['objectex1']:
			# packed as value and then key
			header = data[1]
			numElements = data[1]
			element = 0
		...
		if data[0] == self.EVEMarshalOpCodes['tuple'] or data[0] == self.EVEMarshalOpCodes['dict']:
			...
			while currentElement < numElements:
				if data[tupleSize] == self.EVEMarshalOpCodes['none'] or data[tupleSize] == self.EVEMarshalOpCodes['minusone'] or data[tupleSize] == self.EVEMarshalOpCodes['zero'] or data[tupleSize] == self.EVEMarshalOpCodes['one'] or data[tupleSize] == self.EVEMarshalOpCodes['zerodouble']:
					...
					tupleSize += 9
				elif data[tupleSize] == self.EVEMarshalOpCodes['longstring'] or data[tupleSize] == self.EVEMarshalOpCodes['stringtableitem'] or data[tupleSize] == self.EVEMarshalOpCodes['emptystring'] or data[tupleSize] == self.EVEMarshalOpCodes['stringempty'] or data[tupleSize] == self.EVEMarshalOpCodes['shortstring'] or data[tupleSize] == self.EVEMarshalOpCodes['charstring']:
					tupleSize += data[tupleSize + 1] + 2
				elif data[tupleSize] == self.EVEMarshalOpCodes['utf8string']:
					tupleSize += data[tupleSize + 1] + 2
				elif data[tupleSize] == self.EVEMarshalOpCodes['single_char_str']:
					tupleSize += 3
				else:
			...
			logging.error("Cannot get size of tuple with opcode %s", data[0])
